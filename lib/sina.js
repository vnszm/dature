...
async function articleList(uid, dir, cookie) {
  ...
  await writeFile(path.join(dir, 'data.js'), 'window.blog=' + JSON.stringify({ uid, title, link, urls }), 'utf8') // 写入数据文件data.js
...
async function article(n, len, link, dir, cookie) {
  ...
  // const filename = path.join(dir, path.basename(link, '.html').replace('_', '/') + '.js')
  const art = { title, date, cate, tags, imgs, link, content }
...
module.exports = async function extract(uid, dir, cookie) {
  ...
  for (let url of urls) {
    await article(n++, urls.length, url.href, dir, cookie)
  }
}
